...
function newModel({ className, connection, parentClass }) {
    var _a;
    return _a = class DBSpecializedModel extends (parentClass || class Object {
        }) {
            ...
            static prepareFilterOnSemesterOrders(where, semesterOrders) { }
            // tslint:disable-next-line:no-empty
            static onCreate(revisionMetadata) {
                return __awaiter(this, void 0, void 0, function* () { });
            }
            // tslint:disable-next-line:no-empty
            static onUpdate(revisionMetadata) {
                return __awaiter(this, void 0, void 0, function* () { });
            }
            // tslint:disable-next-line:no-empty
            static onDelete(revisionMetadata) {
                return __awaiter(this, void 0, void 0, function* () { });
            }
            static create(bodies, commitId, { getId } = { getId: false }) {
                return __awaiter(this, void 0, void 0, function* () {
                    if (bodies.length === 0)
                        return getId ? [] : null;
                    ...
                    const [result] = yield this.connection.query(sql, bindings);
                    const revisionMetadata = { insertRevisionId: result.insertId, affectedRows: result.affectedRows };
                    yield this.onCreate(revisionMetadata);
                    if (!getId)
                        return null;
                    return this.getIdsOfResourcesFromRevisionMetadata(revisionMetadata);
                });
            }
            static getIdsOfResourcesFromRevisionMetadata({ insertRevisionId, affectedRows }) {
                return __awaiter(this, void 0, void 0, function* () {
                    const [rows] = yield this.connection.query('SELECT id FROM `' + this.className + 'Revision` WHERE revisionId >= ? AND revisionId < ?', [insertRevisionId, insertRevisionId + affectedRows]);
                    return rows.map((_) => _.id);
                });
            }
            static update(bodiesParam, commitId) {
                return __awaiter(this, void 0, void 0, function* () {
                    if (bodiesParam.length === 0)
                        return null;
                    const bodies = yield Promise.all(bodiesParam.map((bodyParam) => __awaiter(this, void 0, void 0, function* () {
                        const body = Object.assign({}, yield this.q.find(bodyParam.id), bodyParam);
                        body.editDate = Date.now();
                        body.editCommitId = commitId;
                    ...
                    const result = yield this.connection.query(sql, bindings);
                    const revisionMetadata = { insertRevisionId: result.insertId, affectedRows: result.affectedRows };
                    yield this.onUpdate(revisionMetadata);
                    return result;
                });
            }
            static delete(ids, commitId) {
                return __awaiter(this, void 0, void 0, function* () {
                    if (ids.length === 0)
                        return null;
                    const bodies = yield this.q.findMany(ids);
                    ...
                    const result = yield this.connection.query(sql, bindings);
                    const revisionMetadata = { insertRevisionId: result.insertId, affectedRows: result.affectedRows };
                    yield this.onDelete(revisionMetadata);
                    return result;
                });
            }
            static toObject(body, doJointure, exclude) {
                const prefix = this.className[0].toLocaleLowerCase() + this.className.substring(1);
                const Constructor = this.prototype.constructor;
                const obj = new Constructor(body[`${prefix}.id`], body[`${prefix}.editCommitId`], body[`${prefix}.editDate`]);
                for (const field of this.fields) {
                    if (exclude.includes(field.name))
                        continue;
                    obj[field.name] = field.deserialize(body[`${prefix}.${field.name}`]);
                    if (field.foreignKey && field.autoJoin && doJointure) {
                        if (!obj.jointure)
                            obj.jointure = {};
                        const joinName = field.name.replace('Id', '');
                        obj.jointure[joinName] = field.foreignKey.toObject(body, doJointure, exclude);
                    }
                }
                return obj;
            }
            static prepareField(field) {
                ...
                if (typeof field.sqlSetter === 'undefined')
                    field.sqlSetter = '?';
                if (typeof field.serialize === 'undefined')
                    field.serialize = (_) => _;
                if (typeof field.deserialize === 'undefined')
                    field.deserialize = (_) => _;
                ...
                if (typeof field.autoJoin === 'undefined')
                    field.autoJoin = false;
                return field;
            }
            ...
            static registerFieldForeignKey(name, foreignKey, { autoJoin, nullable } = { autoJoin: false, nullable: false }) {
                if (!foreignKey)
