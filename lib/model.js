...
function newModel({ className, connection }) {
    ...
    return _a = class DBSpecializedModel {
            ...
            static async create(bodies, commitId, { getId, conn } = {}) {
                getId || (getId = false);
                ...
                try {
                    ...
                    const instances = failureExplicitor.length ? bodies.map(body => Object.assign(new this(null, body.editCommitId, body.editDate), body)) : [];
                    for (const instance of instances)
                        for (const method of failureExplicitor)
                            await instance[method]('create', err);
                    throw err;
                }
            }
            static async getIdsOfResourcesFromRevisionMetadata({ insertRevisionId, affectedRows }, conn = this.connection()) {
                const [rows] = await conn.query('SELECT id FROM `' + this.className + 'Revision` WHERE revisionId >= ? AND revisionId < ?', [insertRevisionId, insertRevisionId + affectedRows]);
                return rows.map((_) => _.id);
            }
            static async update(bodiesParam, commitId, conn = this.connection()) {
                ...
                try {
                    ...
                    // methods starting with 'onSaveFailed' can throw their own error to explicit validation failure.
                    const failureExplicitor = Object.getOwnPropertyNames(this.prototype).filter(_ => _.startsWith('onSaveFailed'));
                    const instances = failureExplicitor.length ? bodies.map(body => Object.assign(new this(body.id, body.editCommitId, body.editDate), body)) : [];
                    for (const instance of instances)
                        for (const method of failureExplicitor)
                            await instance[method]('update', err);
                    throw err;
                }
            }
            static async delete(ids, commitId, conn = this.connection()) {
                if (ids.length === 0)
                    return null;
